...
class FriendsContactActivity : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityFriendsContactBinding
    // creating adapter
    private var adapter = ContactAdapter()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFriendsContactBinding.inflate(layoutInflater)
        setContentView(binding.root)
        setUpData(binding)

        // Action Bar
        actionBar?.title = "Friends Contact Lists"
        supportActionBar?.title = "Friends Contact Lists"

    }

    // Set up recyclerview
    fun setUpData(binding: ActivityFriendsContactBinding) {
        binding.friendRecyclerView.adapter = adapter

        // item decoration
        binding.friendRecyclerView.addItemDecoration(DividerItemDecoration(this, LinearLayout.VERTICAL))
